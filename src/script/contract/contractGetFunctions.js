...
module.exports = {
  ...
  game: function(index) {
    return Contract.methods.games(index)
    .call()
    .then(game => {
      game.playerX = game.playerX.toLowerCase();
      game.playerO = game.playerO.toLowerCase();
  ...
  waitingGame: function(amount, index) {
    return Contract.methods.waitingGames(BigNumber(amount).multipliedBy(10 ** 18).toString(), index)
    .call()
    .then(v => parseInt(v))
    .catch(ex => 0);
  }
